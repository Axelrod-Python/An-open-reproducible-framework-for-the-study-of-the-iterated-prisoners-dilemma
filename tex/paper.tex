...
\cite{Axelrod-Pythonprojectteam2015} now boasts over 135 strategies that are
...
(\url{https://pypi.python.org/pypi}). The package name is \texttt{axelrod} and
can thus be installed by calling: \texttt{pip install axelrod}. This ensures it
can be used on all major operating systems (Windows, OS X and Linux).
...
features are added to the library to ensure compatibility. Thus, when submitting
a strategy, a simple test is required which ensures the strategy behaves as
expected.
Full contribution guidelines can be found in the documentation.  As an
example, Figures~\ref{fig:grudger} and~\ref{fig:grudger_test} show the source
code for the Grudger strategy as well as its corresponding test.
...
responsible for coordinating the play of generated matches. This generation of
matches is the responsibility of a match generator class which is designed in
such a way as to be easily modifiable to create new types of matches. This is
described fuller in a tutorial in the documentation which shows how to easily
create a tournament where players only play each other with probability 0.5.
This will be discussed further in the reuse section of this paper.
...
To date the library has had contributions from 25 contributors from a variety
of backgrounds which are not solely academic. These contributions have been in
terms of strategies. One strategy is the creation of an undergraduate
mathematics student with little prior knowledge of programming. Multiple other
strategies were written by a 15 year old secondary school student. Both of
these students are authors of this paper. As well as these strategy
contributions, vital architectural improvements to the library itself have also
been received.
...
with the results of \cite{Stewart2012} are due to stochastic behaviour.
...
The current winning strategy is a novel strategy: Looker Up. This is a
...
The library is continuously tested for compatibility with Python 2.7, 3.4 and 3.5.
...
Add more here: point to previous discussion about new strategies, new
tournaments, tests etc\dots
