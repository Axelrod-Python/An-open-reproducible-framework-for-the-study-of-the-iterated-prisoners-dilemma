...
\noindent
    ...
    documented and test code. The goal of the library is to provide such
    a resource, with facilities the design new strategies and interactions
    between them, as well as conducting tournaments and ecological simulations
    for population of strategies.

\noindent
    With a growing collection of 120 strategies the library is a also a platform
...
two 2004 anniversary tournaments (ref?), and
...
describe a computer tournament that has been a imajor influence on
...
although any satisfying the conditions in \ref{equ:one-shot} will yield similar results.
Axelrod's tournaments (and further implementations of these) are sometimes
referred to as Iterated Prisoner's Dilemma (IPD) tournaments. An incomplete
overview of published tournaments is given in Table~\ref{tab:tournaments}.
...
\begin{table}[!hbtp]
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \caption{An overview of published tournaments (specify what "type" means)}\label{tab:tournaments}
\end{table}
...
impossible to reproduce as the source code is forever lost, and this library aims
...
taking in to account the recent play. The actual winning strategy is an instance
